...
export const consolidatedData = pgTable("consolidated_data", {
	...
	financialAcquisitionPricePerSqm: numeric("financial_acquisition_price_per_sqm", { precision: 10, scale:  2 }),
	...
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow().notNull(),
	financialMarketPricePerSqm: numeric("financial_market_price_per_sqm", { precision: 10, scale:  2 }),
}, (table) => [
	foreignKey({
			columns: [table.projectId],
			foreignColumns: [projects.id],
...
export const companies = pgTable("companies", {
	...
	reputationJustification: text("reputation_justification"),
}, (table) => [
	foreignKey({
			columns: [table.projectId],
			foreignColumns: [projects.id],
...
export const strengthsAndWeaknesses = pgTable("strengths_and_weaknesses", {
	...
	type: strengthWeaknessType().notNull(),
